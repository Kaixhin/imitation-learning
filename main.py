...
def main(cfg: DictConfig) -> None:
  ...
  assert cfg.imitation_algorithm in ['AIRL', 'DRIL', 'FAIRL', 'GAIL', 'GMMIL', 'PUGAIL', 'RED', 'BC', 'PPO']
  # General setup
  ...
  agent_optimiser = optim.RMSprop(agent.parameters(), lr=cfg.reinforcement.agent_learning_rate, alpha=0.9)
  # Set up imitation learning components
  if cfg.imitation_algorithm in ['AIRL', 'DRIL', 'FAIRL', 'GAIL', 'GMMIL', 'PUGAIL', 'RED']:
    if cfg.imitation_algorithm == 'AIRL':
    ...
    elif cfg.imitation_algorithm == 'DRIL':
      discriminator = Actor(state_size, action_size, cfg.model.hidden_size, dropout=0.1)
    elif cfg.imitation_algorithm in ['FAIRL', 'GAIL', 'PUGAIL']:
      discriminator = GAILDiscriminator(state_size + (1 if cfg.imitation.absorbing else 0), action_size, cfg.model.hidden_size, state_only=cfg.imitation.state_only, forward_kl=cfg.imitation_algorithm == 'FAIRL')
    elif cfg.imitation_algorithm == 'GMMIL':
    ...
    elif cfg.imitation_algorithm == 'RED':
    ...
    if cfg.imitation_algorithm in ['AIRL', 'DRIL', 'FAIRL', 'GAIL', 'PUGAIL', 'RED']:
      discriminator_optimiser = optim.RMSprop(discriminator.parameters(), lr=cfg.imitation.il_learning_rate)
  ...
  if cfg.imitation_algorithm in ['AIRL', 'FAIRL', 'GAIL', 'PUGAIL']: policy_trajectory_replay_buffer = deque(maxlen=cfg.imitation.imitation_replay_size)
  ...
  for step in pbar:
    ...
    if cfg.imitation_algorithm in ['BC', 'DRIL', 'RED']:
      if step == 1:
        for _ in tqdm(range(cfg.imitation.imitation_epochs), leave=False):
          if cfg.imitation_algorithm == 'BC':
    ...
    if cfg.imitation_algorithm != 'BC':
      ...
      if len(trajectories) >= cfg.training.batch_size:
        ...
        if cfg.imitation_algorithm in ['AIRL', 'DRIL', 'FAIRL', 'GAIL', 'GMMIL', 'PUGAIL', 'RED']:
          # Train discriminator
          if cfg.imitation_algorithm in ['AIRL', 'FAIRL', 'GAIL', 'PUGAIL']:
            ...
            for _ in tqdm(range(cfg.imitation.imitation_epochs), leave=False):
              adversarial_imitation_update(cfg.imitation_algorithm, agent, discriminator, expert_trajectories, TransitionDataset(policy_trajectory_replays), discriminator_optimiser, cfg.training.batch_size, cfg.imitation.absorbing, cfg.imitation.r1_reg_coeff, cfg.get('pos_class_prior', 0.5), cfg.get('nonnegative_margin', 0))
    ...
    elif cfg.imitation_algorithm == 'BC' and cfg.check_time_usage: break
  ...
  if cfg.imitation_algorithm in ['AIRL', 'DRIL', 'FAIRL', 'GAIL', 'PUGAIL', 'RED']: torch.save(discriminator.state_dict(), 'discriminator.pth')
  torch.save(metrics, 'metrics.pth')


  env.close()
  return sum(recent_returns) / float(1 if cfg.imitation_algorithm == 'BC' else cfg.evaluation.average_window)
